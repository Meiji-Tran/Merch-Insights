...
-- Find customers who purhcased irrigation projects and when they started
WITH base AS (
     SELECT
        dw_commercial_account_id,
    ...
    where 1=1
    and i.item_sub_department_name = '400 RODENT PEST CONTROL'
    group by dw_commercial_account_id
-- What did these customers purchase, centring time around their project start date
    ...
    SELECT
        ...
        (SELECT COUNT(DISTINCT dw_commercial_account_id) FROM base)  AS irrigation_projects_consumer,
        COUNT(DISTINCT stl.dw_commercial_account_id) AS customers,
        SUM(stl.total_include_gst_amount) AS sales
    FROM bdwprd_cds.sales.sales_transaction_line_fct stl
        INNER JOIN bdwprd_cds.item.item_dim i
            ON i.dw_item_id = stl.dw_item_id
        INNER JOIN base b
        	ON b.dw_commercial_account_id = stl.dw_commercial_account_id
    WHERE 1=1
...
-- Get customer counts separately
    ...
    SELECT
    	DATEDIFF(WEEK, b.first_purchase_date, stl.transaction_date) AS weeks_since_purchase,
        COUNT(DISTINCT stl.dw_commercial_account_id) AS total_customers
    FROM bdwprd_cds.sales.sales_transaction_line_fct stl
        INNER JOIN bdwprd_cds.item.item_dim i
            ON i.dw_item_id = stl.dw_item_id
        INNER JOIN base b
...
-- Calculate the usual sales/customer counts at each PFT level for indexing
    ...
    SELECT
        ...
        COUNT(DISTINCT stl.dw_commercial_account_id) AS customers,
        SUM(total_include_gst_amount) AS sales,
