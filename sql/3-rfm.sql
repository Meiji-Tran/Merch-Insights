...
INNER JOIN bdwprd_apps.customer.rfm_features_annual rfm
...
WHERE 1=1
    AND rfm.end_period_end_date = DATE_TRUNC(WEEK, (SELECT MIN(rollout_date) FROM bdwprd_de.ia_merch_de.ev_rollout)) -- Select most relevant model run
    AND rfm.annual_segment ILIKE '%Spend%' -- Only include the four core segments
