-- RFM Annual Segment Indexing by {level}

WITH rfm_seg AS (
    SELECT
        flybuys_membership_number_hash,
        annual_segment
    FROM bdwprd_apps.customer.rfm_features_annual
    WHERE end_period_end_date = DATE_TRUNC(WEEK, (
        SELECT MIN(rollout_date)
        FROM bdwprd_de.ia_merch_de.ev_rollout
    ))
      AND annual_segment ILIKE '%Spend%'
),
...
range_trx AS (
    ...
    JOIN rfm_seg rfm
        ON fa.flybuys_membership_number_hash = rfm.flybuys_membership_number_hash
    WHERE 1=1
...
-- Totals per {level} × annual segment
seg_level AS (
    SELECT
        {level},
        annual_segment,
...
JOIN seg_total st
    ON sl.annual_segment = st.annual_segment
JOIN overall ovr
    ON 1=1
ORDER BY
    sl.{level},
