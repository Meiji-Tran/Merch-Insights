...
, commercial_demo_info as (
    ...
    prim.commercial_industry_segment_report_group_code AS PRIMARY_SEGMENT,
    psp.PSP_CUST_PREMIUMNESS
	from bdwprd_cds.commercial.commercial_account_dim pp
	inner join BDWPRD_CDS.COMMERCIAL.COMMERCIAL_ACCOUNT_FCT accts
	   ON pp.dw_commercial_account_id = accts.dw_commercial_account_id
    INNER JOIN BDWPRD_CDS.COMMERCIAL.COMMERCIAL_INDUSTRY_SEGMENT_REPORT_GROUP_DIM prim
        on prim.dw_commercial_industry_segment_report_group_id=accts.dw_commercial_industry_segment_report_group_id
    LEFT JOIN BDWPRD_DE.MERCH_PREMIUM_SPEND_DE.PSP_FINAL_CUSTOMER_AU psp
...
IFF(UPPER(PRIMARY_SEGMENT) ILIKE '%UNCLASSIFIABLE%' or primary_segment is null,'Unclassifiable',PRIMARY_SEGMENT) AS SEGMENT,
